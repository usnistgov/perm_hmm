...
class MyTestCase(unittest.TestCase):
    ...
    def test_something(self):
        ...
        iep = self.bs.simulate(self.num_bins, num_samples)
        x, training_data = self.bhmm.sample((num_train, self.num_bins))
        _ = train_ic(self.ic, training_data, x[..., 0],
                     len(self.bhmm.initial_logits))
        pp = self.bs.simulate(self.num_bins, num_samples, perm_selector=self.perm_selector)
        nop, d = self.bs.simulate(self.num_bins, num_samples, verbosity=1)
        ...
        nop = self.bs.all_classifications(self.num_bins)
        pp = self.bs.all_classifications(self.num_bins, perm_selector=self.perm_selector)
