...
from perm_hmm.return_types import ExperimentParameters, PermutedParameters
...
def exact_rates(phmm: PermutedDiscreteHMM, testing_states, num_bins, perm_selector, classifier=None, num_ratios=20, verbosity=0):
    experiment_parameters = ExperimentParameters(
        PermutedParameters(
            phmm.initial_logits,
            phmm.transition_logits,
            phmm.observation_dist._param,
            phmm.possible_perms,
        ),
        testing_states,
...
def empirical_rates(phmm: PermutedDiscreteHMM, testing_states, num_bins, perm_selector, classifier=None, num_ratios=20, num_train=1000, num_samples=1000, confidence=.95, verbosity=0):
    ...
    ground_truth, training_data = phmm.sample((num_train,))
    _ = train_ic(ic, testing_states, training_data, ground_truth, len(phmm.initial_logits), num_ratios=num_ratios)
    ...
    if verbosity:
        pp, pd = pp
    class_break_ratio = ic.classify(d["data"], testing_states, verbosity=True)
...
def main(args):
    ...
    if args.save_raw_data:
        verbosity = 2
    if args.exact:
        d = exact_rates(perm_hmm, bright_or_dark, num_bins, perm_selector, verbosity=verbosity)
    elif args.approximate:
        d = empirical_rates(perm_hmm, bright_or_dark, num_bins, perm_selector, num_train=args.num_training_samples, num_samples=args.num_samples, verbosity=verbosity)
    else:
        d = {}
    print("Done.\n")
    d[b"args"] = args
