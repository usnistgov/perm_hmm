"""
Simulations for an experiment with binary outcomes.
"""
import warnings
...
class ExactResults(NamedTuple):
    interrupted_postprocessor: InterruptedExactPostprocessor
    """:py:class:`bayes_perm_hmm.postprocessing.InterruptedExactPostprocessor`
    The postprocessor for the interrupted scheme.
    """
    naive_postprocessor: PostDistExactPostprocessor
    """:py:class:`bayes_perm_hmm.postprocessing.PostDistExactPostprocessor`
    The postprocessor for data generated by an HMM, here for the data without
    permutations.
    """
    bayes_postprocessor: PostDistExactPostprocessor
    """:py:class:`bayes_perm_hmm.postprocessing.PostDistExactPostprocessor`
    The postprocessor for data generated by an HMM, here for the data with
    permutations.
    """
    runs: ExactRun
    """:py:class:`ExactRun`
    The data which is analyzed by the postprocessors.
    """


class EmpiricalResults(NamedTuple):
    """
    The results of an empirical misclassification experiment.

    .. seealso:: the return type of
        :py:meth:`BernoulliSimulator.empirical_simulation`
    """
    interrupted_postprocessor: InterruptedEmpiricalPostprocessor
    """
    :py:class:`bayes_perm_hmm.postprocessing.InterruptedEmpiricalPostprocessor`
    The postprocessor for the data returned by the
    :py:class:`bayes_perm_hmm.interrupted.InterruptedClassifier`
    run on the data generated by the HMM without permutations.
    """
    naive_postprocessor: PostDistEmpiricalPostprocessor
    """:py:class:`bayes_perm_hmm.postprocessing.PostDistEmpiricalPostprocessor`
    The postprocessor for the data taken without permutations.

    The data analyzed in this object is the same data analyzed in
    `interrupted_postprocessor`.
    """
    bayes_postprocessor: PostDistEmpiricalPostprocessor
    """:py:class:`bayes_perm_hmm.postprocessing.PostDistEmpiricalPostprocessor`
    The postprocessor for the data taken with permutations.
    """
    runs: EmpiricalRun
    """:py:class:`EmpiricalRun`
    All the data generated in this simulation.
    """


class BernoulliSimulator(object):
    """
    ...
    def exact_train_ic(self, num_ratios=20):
        data = torch.stack(
            [num_to_data(num, self.num_bins) for num in range(2**self.num_bins)]
        )
    ...
    def exact_simulation(self):
        ...
        bayes_result = self.bayes_hmm.get_perms(data)
        perm = bayes_result.optimal_perm
        bayes_lp = self.bayes_hmm.log_prob_with_perm(perm, data)
        ...
        bayes_ep = PostDistExactPostprocessor(
            bayes_lp,
            bayes_result.history.partial_post_log_init_dists[..., -1, :],
            self.bayes_hmm.initial_logits,
            self.testing_states,
        )
    ...
    def empirical_simulation(self, num_samples):
        """
        ...
        bayes_ep = PostDistEmpiricalPostprocessor(
            results.bayes.sample.states[..., 0],
            self.testing_states,
            self.bayes_hmm.initial_logits.shape[-1],
            results.bayes.bayesian.history.partial_post_log_init_dists[..., -1, :],
        )
